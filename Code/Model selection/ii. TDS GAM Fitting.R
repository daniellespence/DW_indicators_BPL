...
df1 <- df1[complete.cases(df$TDS),]
# 284 obs
df1 <- filter(df1, DIN != 0) # 284
df1 <- filter(df1, SRP != 0) # 198 obs
...
DINplot <- ggplot(DIN.pdatnorm, aes(x = DIN, y = Fitted)) +
  ...
  #         show.legend = FALSE) +
  #geom_abline(slope = 0, intercept = meanTDS, linetype="dotted") +
...
tempplot <- ggplot(temp.pdatnorm, aes(x = W_temp, y = Fitted)) +
  ...
  geom_rug(aes(x=W_temp), data = df1, stat = "identity", position = "identity", 
           sides = "b", na.rm = FALSE, show.legend = NA, inherit.aes = FALSE, alpha=0.3) +
  xlab(expression(paste("Water temperature (" , degree*C,")"))) +ylab(expression(paste("TDS"~"("*"mg"~L^{-1}*")")))
...
QLDplot <- ggplot(QLD.pdatnorm, aes(x = QLD, y = Fitted)) +
 ...
 geom_rug(aes(x=QLD), data = df1, stat = "identity", position = "identity", 
           sides = "b", na.rm = FALSE, show.legend = NA, inherit.aes = FALSE, alpha=0.3) +
  xlab(expression(paste("QLD (",m^3, "/", s,")"))) + ylab(expression(paste("TDS"~"("*"mg"~L^{-1}*")")))
...
comboplot <- ggplot(SOI_3MON_AVG.pdatnorm, aes(x = PDO_3MON_AVG, y = SOI_3MON_AVG, z=TDS)) + 
  ...
  geom_raster(aes(fill=TDS)) + # change to turn grey background into nothing
...
ggsave('output/TDS GAM_Response scale.png', p_all, height = 10, width  = 10)
...
df1<- df1%>%
  select(date, year, nMonth, DOY, TDS, SRP, DIN, QLD, W_temp, SOI_3MON_AVG, PDO_3MON_AVG)
df1<- na.exclude(df1)
...
ggsave('output/TDS GAM_partial effects seasonal.png', p_allEFF, height = 12, width  = 12)
