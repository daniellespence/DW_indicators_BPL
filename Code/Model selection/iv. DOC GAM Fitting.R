...
library(pacman)
p_load(tidyverse, ggplot2, mgcv, gratia, readr, GGally, dplyr, lubridate, plyr,
...
df1 %>%
  dplyr::select(DOC, SRP,  W_temp,  DIN, QLD) %>%
  gather() %>%
  ggplot(aes(value)) +
...
df1 %>%
  dplyr::select(DOC, SRP,  W_temp,  DIN) %>%
  gather() %>%
  ggplot(aes(value)) +
  facet_wrap(~ key, scales = "free") +
  geom_histogram()
...
dfx<-df1%>%
  select(DOC, SRP, DIN, QLD, QWS, QBP, W_temp, PDO_3MON_AVG, SOI_3MON_AVG, pH)
...
m <- gam(DOC ~ s(SRP) +  
         ...
         select = TRUE,
         data = df1, method = "REML", tw(link = "log"))
...
AIC(m, m1, m2, m3) #m2 (tw family is best fit)



##--- check which variables should stay/be removed

nullvars <- c('DOC', "SRP", "DIN", "QLD", "W_temp", "PDO_3MON_AVG", "SOI_3MON_AVG", "year", "DOY")
...
summary(m2) # Deviance explained = 54, REML = 1289, r2 = 0.52
# all predictors are significant, n=707
...
ggsave('output/DOC GAM residuals.png', p2, height = 10, width  = 10)
...
## i. Appraise model fit
appraise(m2$gam, point_col = 'steelblue', point_alpha = 0.5, n_bins = 'fd') & 
  theme(plot.tag = element_text(face = 'bold')) & theme_bw()

p1<- draw(m1$gam, residuals = TRUE,  select = smooths(m1$gam)[1:5])& theme_bw()
p1

#ggsave('output/DOC GAM_ autocorrelation no interaction SOI.png', p1, height = 10, width  = 10)
...
DINplot <- ggplot(DIN.pdatnorm, aes(x = DIN, y = Fitted)) +
  ...
  #         show.legend = FALSE) +
  #geom_abline(slope = 0, intercept = meanDOC, linetype="dotted") +
...
df1<- df1%>%
  select(date, year, nMonth, DOY, DOC, SRP, DIN, QLD, W_temp, SOI_3MON_AVG, PDO_3MON_AVG)
df1<- na.exclude(df1)
...
ggsave('output/DOC GAM_partial effects long-term.png', p_allEFF, height = 12, width  = 12)
