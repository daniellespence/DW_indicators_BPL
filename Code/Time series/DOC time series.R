...
layout(matrix(1:2, ncol = 2))
res <- resid(m3$lme, type = "normalized")
...
saveRDS(p2, "DOC_AC_residuals.rds")

ggsave('output/DOC AC GAM residuals.png', p2, height = 10, width  = 10)
...
DOY.pred <- predict(m3$gam, newdata = new_data_DOY, type = "terms", se.fit = TRUE)
...
DOYplot <- ggplot(DOY.pdatnorm, aes(x = DOY, y = Fitted)) +
  ...
  xlab(expression(paste("DOY "))) + ylab(expression(paste("DOC (mg/L)")))
...
year.pred <- predict(m3$gam, newdata = new_data_year, type = "terms", se.fit = TRUE)
...
yearplot <- ggplot(year.pdatnorm, aes(x = year, y = Fitted)) +
  ...
  scale_y_continuous(expand = c(0, 0)) +  # Remove padding on x-axis
 xlab(expression(paste("Year"))) + ylab(expression(paste("DOC (mg/L)")))
