...
df<- df%>%
  filter(parameter %in% c("Nitrate","Ammonia N","Phosphate (ortho)","Phosphate (total)","Temperature","Organic N", "Turbidity"))
...
df$date_ymd<- as.Date(df$date_ymd, "%m/%d/%Y")
 ...
 #       dplyr::filter(parameter == "Turbidity") %>%
 #   dplyr::summarise(
...
missing_2001 <- read_excel("Data/Raw data/ROUTINE LAB DATA 2001(v2).xlsx",
  ...
  filter(Parameters %in% c("Nitrate","Ammonia N","Phosphate (ortho)","Phosphate (total)","Temperature","Organic N", "Turbidity")) %>% 
  select(-c("...3":"...4")) %>% 
...
d2<- bp_longterm_infill %>%
 ...
 #             median = median(Turbidity, na.rm = TRUE),
 #             sd = sd(Turbidity, na.rm = TRUE),
...
# df %>% filter(is.na(NH3_mg.L)) #874 NAs
# df %>% filter(is.na(NO3_mg.L)) #791 NAs
# df %>% filter(is.na(SRP_ug.L)) #871 NAs
# df %>% filter(is.na(Turbidity)) #3 NAs
...
sum(full_df$NO3_mg.L == 0, na.rm = TRUE) #533
...
# > print(parameter_summary)
# mean median       sd  min max count
# 1 4.099893   2.64 4.567413 0.35  77  2030


write.csv(full_df, file="data/bpgamdataCLEAN_Turb.csv", row.names=F)
